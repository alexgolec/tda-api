...
class Service(Enum):
    ...
    ACTIVES_NYSE = auto()
    ACTIVES_OTCBB = auto()
    ACTIVES_OPTIONS = auto()
    FOREX_BOOK = auto()
    FUTURES_BOOK = auto()
    LISTED_BOOK = auto()
    NASDAQ_BOOK = auto()
    OPTIONS_BOOK = auto()
    FUTURES_OPTIONS_BOOK  = auto()
    CHART_EQUITY = auto()
    CHART_FUTURES = auto()
    CHART_HISTORY_FUTURES  = auto()
    QUOTE = auto()
    LEVELONE_FUTURES = auto()
    LEVELONE_FOREX = auto()
    LEVELONE_FUTURES_OPTIONS = auto()
    OPTION = auto()
    LEVELTWO_FUTURES = auto()
    NEWS_HEADLINE = auto()
    NEWS_STORY = auto()
    NEWS_HEADLINE_LIST = auto()
    STREAMER_SERVER = auto()
    TIMESALE_EQUITY = auto()
    TIMESALE_FUTURES = auto()
    TIMESALE_FOREX = auto()
    TIMESALE_OPTIONS = auto()
    ...
    async def handle_message(self):
        ...
        if 'data' in msg:
            for d in msg['data']:
                ...
                if len(handlers) > 0:
                    mutated_d = self._field_normalizer(service)(d)
                    for handler in self._handlers[service]:
                        ...
                        handler(copy.deepcopy(mutated_d))

        # On notify, unmodified versions of the data are passed.
        # We also don't pass copied versions of the data to them, so if they
        # mutate it, bad things could happen.
        if 'notify' in msg:
            for d in msg['notify']:
                if 'heartbeat' in d:
                    pass
